document.addEventListener("DOMContentLoaded", () => {
    ...
    const translations = {
        en: {
            ...
            suggestionsTitle: "Smart Suggestions", healthTitle: "Health & Wellness", compareTitle: "Compare Cities", forecastTitle: "5-Day Forecast", chartTitle: "Temperature Trend",
            ...
            compareBtnText: "Compare", compareP1: "First city...", compareP2: "Second city...",
        ...
        hi: {
            ...
            suggestionsTitle: "स्मार्ट सुझाव", healthTitle: "स्वास्थ्य और कल्याण", compareTitle: "शहरों की तुलना", forecastTitle: "5-दिन का पूर्वानुमान", chartTitle: "तापमान का रुझान",
            ...
            compareBtnText: "तुलना करें", compareP1: "पहला शहर...", compareP2: "दूसरा शहर...",
        ...
        mr: {
            ...
            suggestionsTitle: "स्मार्ट सूचना", healthTitle: "आरोग्य आणि निरोगीपणा", compareTitle: "शहरांची तुलना", forecastTitle: "५-दिवसांचा अंदाज", chartTitle: "तापमान कल",
            ...
            compareBtnText: "तुलना करा", compareP1: "पहिले शहर...", compareP2: "दुसरे शहर...",
    ...
    const updateStaticText = () => {
        ...
        if(compareInput1) compareInput1.placeholder = t("compareP1");
        if(compareInput2) compareInput2.placeholder = t("compareP2");
        if(compareBtn) compareBtn.textContent = t("compareBtnText");
    };
    ...
    const handleCompare = async () => {
        const city1 = compareInput1.value.trim();
        const city2 = compareInput2.value.trim();
        
        if(!city1 || !city2) return;
        
        compareResultEl.innerHTML = '<i class="fa-solid fa-spinner fa-spin"></i>';
        ...
        try {
            ...
            const [loc1, loc2] = await Promise.all([getCoords(city1), getCoords(city2)]);
            
            if(!loc1 || !loc2) {
                compareResultEl.innerHTML = `<p style="color:red">One or both cities not found.</p>`;
                return;
            }

            const getWeather = async (l) => {
                const weatherUrl = `https://api.open-meteo.com/v1/forecast?latitude=${l.latitude}&longitude=${l.longitude}&current=temperature_2m,weather_code`;
                const aqiUrl = `https://air-quality-api.open-meteo.com/v1/air-quality?latitude=${l.latitude}&longitude=${l.longitude}&current=us_aqi`;
                const [wr, ar] = await Promise.all([fetch(weatherUrl), fetch(aqiUrl)]);
                const [w, a] = await Promise.all([wr.json(), ar.json()]);
                return { weather: w, aqi: a.current ? a.current.us_aqi : null };
            };

            const [res1, res2] = await Promise.all([getWeather(loc1), getWeather(loc2)]);
            
            const createCard = (loc, data) => {
                const { weather, aqi } = data;
                const icon = (weatherBaseMap[weather.current.weather_code] || { icon: "fa-cloud" }).icon;
                const temp = Math.round(isCelsius ? weather.current.temperature_2m : toFahrenheit(weather.current.temperature_2m));
                
                let aqiHtml = "AQI: --";
                if(aqi !== null) {
                    let color = "#28a745";
                    if (aqi > 50) color = "#ffc107";
                    if (aqi > 100) color = "#fd7e14";
                    if (aqi > 150) color = "#dc3545";
                    aqiHtml = `<span style="color:${color}; font-weight:700;">AQI: ${aqi}</span>`;
                }

                return `
                    <div class="daily-card" style="min-width: 180px; flex: 1;">
                        <p class="day" style="font-size: 1.2rem;">${loc.name}</p>
                        <i class="fa-solid ${icon}" style="font-size: 3rem; margin: 15px 0;"></i>
                        <p class="temps" style="font-size: 2rem;">${temp}°${isCelsius ? 'C' : 'F'}</p>
                        <p class="condition" style="font-size: 1rem;">${mapCodeToDesc(weather.current.weather_code)}</p>
                        <div style="margin-top:10px; font-size:0.9rem; border-top:1px solid rgba(0,0,0,0.1); padding-top:5px;">
                            ${aqiHtml}
                        </div>
                    </div>
                `;
            ...
            compareResultEl.innerHTML = createCard(loc1, res1) + '<div style="display:flex; align-items:center; font-weight:bold; font-size:1.5rem; opacity:0.5;">VS</div>' + createCard(loc2, res2);
        } catch(e) {
            compareResultEl.innerHTML = `<p>Error comparing cities.</p>`;
        }
    };
