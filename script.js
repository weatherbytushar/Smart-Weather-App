    ...
    const handleCompare = async () => {
        ...
        try {
            ...
            const getWeather = async (l) => {
                const r = await fetch(`https://api.open-meteo.com/v1/forecast?latitude=${l.latitude}&longitude=${l.longitude}&current=temperature_2m,weather_code`);
                return r.json();
            };

            const [w1, w2] = await Promise.all([getWeather(loc1), getWeather(loc2)]);
            
            const createCard = (loc, w) => {
                const icon = (weatherBaseMap[w.current.weather_code] || { icon: "fa-cloud" }).icon;
                const temp = Math.round(isCelsius ? w.current.temperature_2m : toFahrenheit(w.current.temperature_2m));
                return `
                    <div class="daily-card" style="min-width: 180px; flex: 1;">
                        ...
                        <p class="condition" style="font-size: 1rem;">${mapCodeToDesc(w.current.weather_code)}</p>
                    </div>
                `;
            };

            compareResultEl.innerHTML = createCard(loc1, w1) + '<div style="display:flex; align-items:center; font-weight:bold; font-size:1.5rem; opacity:0.5;">VS</div>' + createCard(loc2, w2);
        } catch(e) {
